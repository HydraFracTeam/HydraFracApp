...
2. [Компоненты модуля](#компоненты-модуля)
   - [solver/solver_wrapper.py — точка входа](#solversolver_wrapperpy--точка-входа)
   - [solver/solver_new.py — основной алгоритм](#solversolver_newpy--основной-алгоритм)
...
Модуль **solver** реализует ступенчатый алгоритм решения обратной задачи ГДИС после МГРП с последовательным сужением пространства поиска:
...
[Этап 0] Подготовка — интерполяция на логарифмическую сетку
        ↓
[Этап 1] Beam Search → выбор Skin-фактора (top-k) с h-взвешиванием
        ↓
[Этап 2] Перебор N → выбор количества трещин
        ↓
[Этап 3] Beam Search → выбор a/L
        ↓
[Этап 4] Перебор L → выбор полудлины трещины (с интерполяцией)
        ↓
[Этап 5] Восстановление k → аналитический расчёт (X ∝ k)
        ↓
Результат: {Skin, N, L, a/L, k, misfit}
```

**Ключевые принципы:**
- **Два режима невязки:** `misfit_shape` (с нормировкой) для выбора формы, `misfit_aligned` (без нормировки) для восстановления k
- **h-взвешивание** при выборе Skin — мягкий вес по близости толщины пласта
- **Интерполяция** между соседними L для непрерывности результатов
- **Аналитическое восстановление k** из масштабных коэффициентов (X ∝ k)
