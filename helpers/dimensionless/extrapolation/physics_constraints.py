...
def validate_extrapolation(
    ...
    """
    Валидирует физическую корректность экстраполированных значений.
    ...
    Returns:
        (is_valid, issues): Кортеж с флагом валидности и списком проблем
    """
    issues = []
    
    P_ext = np.asarray(P_ext)
    dP_ext = np.asarray(dP_ext)
    Q_ext = np.asarray(Q_ext)
    
    # Проверка давления
    if np.any(P_ext > P_last):
        issues.append(f"Давление растет (макс. {np.max(P_ext):.2f} > {P_last:.2f})")
    
    if np.any(P_ext < 0):
        issues.append("Отрицательное давление")
    
    if len(P_ext) > 1 and np.any(np.diff(P_ext) > 1e-10):
        issues.append("Давление не монотонно убывает")
    
    # Проверка депрессии
    if np.any(dP_ext < 0):
        issues.append("Отрицательная депрессия")
    
    if len(dP_ext) > 0 and dP_ext[0] < dP_last - 1e-6:
        issues.append(f"Депрессия убывает в начале (dP[0]={dP_ext[0]:.2f} < {dP_last:.2f})")
    
    if len(dP_ext) > 1 and np.any(np.diff(dP_ext) < -1e-10):
        issues.append("Депрессия не монотонно возрастает")
    
    # Проверка дебита
    if np.any(Q_ext < 0):
        issues.append("Отрицательный дебит")
    
    if len(Q_ext) > 0:
        max_Q = np.max(Q_ext)
        min_Q = np.min(Q_ext)
        if max_Q > Q_last * 10:
            issues.append(f"Дебит слишком высокий (макс. {max_Q:.2f} > {Q_last * 10:.2f})")
        if min_Q < Q_last * 0.01 and min_Q > 0:
            issues.append(f"Дебит слишком низкий (мин. {min_Q:.2f} < {Q_last * 0.01:.2f})")
