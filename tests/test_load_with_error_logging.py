...
class TestLoadWithErrorLogging:
    ...
    @pytest.mark.load
    def test_error_logging_with_various_data_sizes(self, qapp, error_logger):
        """Тест логирования ошибок с различными размерами данных"""
        from main import MyApp
        ...
        for size in data_sizes:
            ...
            # Создаём данные заданного размера
            time_data = np.linspace(0, 100, size)
            pressure_data = 300 - time_data * 0.5 + np.random.normal(0, 5, size)
            flow_rate_data = 100 - time_data * 0.2 + np.random.normal(0, 3, size)
            
            # Добавляем пропуски (качество данных зависит от размера)
            nan_ratio = 0.1 if size < 100 else 0.05
            n_nans = int(size * nan_ratio)
            nan_indices = np.random.choice(size, n_nans, replace=False)
            pressure_data[nan_indices] = np.nan
            ...
            # Выполняем операции и логируем ошибки
            try:
                app.on_plot_dimensionless_selected()
                QApplication.instance().processEvents()
                
                # Вычисляем качество данных
                quality = 1.0 - (np.sum(np.isnan(pressure_data)) / len(pressure_data))
                
                # Логируем успешное выполнение (можно логировать метрики производительности)
                log_math_error(
                    subsystem="gui",
                    method="plot_dimensionless_selected",
                    error_type="performance",
                    error_value=0.0,  # Нет ошибки, только метрика
                    error_message=f"Успешное выполнение для размера {size}",
                    data_volume=size,
        ...
        # Проверяем, что логи созданы
        df = error_logger.load_errors()
        print(f"\n📊 Всего залогировано записей: {len(df)}")
        if len(df) > 0:
            print(f"   По размерам данных: {df.groupby('data_volume').size().to_dict()}")
